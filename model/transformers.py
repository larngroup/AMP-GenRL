...
class Transformer_Decoder(tf.keras.Model):
    ...
    def build(self, input_shape):
        ...
        self.ffLayer = tf.keras.layers.Dense(self.smiles_vocab_size,activation='softmax')

    
    def call(self, x, training = True):
        mask_loss = tf.cast(~tf.math.equal(x, tf.zeros_like(x)), tf.float32)
        ...
        x = self.ffLayer(x)
        return x, all_weights,mask_loss   # (batch_size, input_seq_len, d_model)
